...
*   `config/IDENTITY.md`: **(MANDATORY)** System identity (Name, Timezone, Version). *(Deprecates identity.yaml)*.
*   `config/ASSOCIATES.md`: **(MANDATORY)** Associate registry and sovereignty hierarchy.
*   `config/MANIFEST.md`: **(MANDATORY)** Signed file-system provenance manifest.
...
*   `skills/`: Externalized "Skill-as-a-File" repository.

## Bootstrap Sequence (First Run)
When initializing a new Soul instance for the first time, follow this sequence:

1.  **Genesis**: Create `config/IDENTITY.md` and `config/ASSOCIATES.md`.
2.  **Imprinting**: Create `config/SOUL.md` (Initial traits and goals).
3.  **Manifest Init**: Generate the initial `config/MANIFEST.md` for the core configuration.
4.  **Ledger Init**: Create the first immutable log entry ("System Birth").
5.  **State Baseline**: Set Operational State to **Content** (Default).
6.  **Validation**: Run the **Pre-Execution Validation** checklist (db, keys, permissions).
7.  **Confirm**: Log "Bootstrap Complete" to Ledger.

## Identity & Sovereignty (Markdown-as-Truth)
To maintain **Ontological Transparency**, the entity’s identity and its relationship with humans are constitutional statements, not hidden binary configs.

*   **Requirement**: Changes to `IDENTITY.md` or `ASSOCIATES.md` MUST be classified as **Constitutional Events**.
*   **The Logic**: If the entity’s name or sovereignty hierarchy changes, it is a traceable event in the **Ledger**, requiring Level 4 MFA approval. The **Bi-Directional Watcher** (Doc 06) ensures that manual edits to these files are synced to the internal substrate only after verification.

## Operational Monitoring (`monitoring.yaml`)
*(Replaces the monitoring section previously in identity.yaml)*
Define the physiological thresholds and heartbeat timing.
